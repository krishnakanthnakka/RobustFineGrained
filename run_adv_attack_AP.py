...
from models import model_AP
from utils import normalize_fn
from utils.adv_utils import attack_fns_AP, get_attack_params
...
def main(args):
    ...
    # --------------------------------------------- Prepare model -----------------------------------------------
    modelpath = './saved_models/{}/{}/{}/{}/{}'.format(args.dataset, args.backbone,
                                                       args.net, args.mode, args.checkpoint)

    ppnet = model_AP.construct_PPNet(base_architecture=NET_ARGS['base_architecture'],
                                     pretrained=False,
    ...
    for it, (data, target) in enumerate(data_loader):
        ...
        adv_img = attack_fns_AP(ppnet_multi, criterion, data, target,
        ...
        # -----------------------------Print Stats---------------------------------------------------------------
        print("Batch: [{}/{}]\t Att_branch Normal: {:.2f}%  \t  "
              "Att_branch Adv: {:.2f}%".format(it, total_batches, (100.0 * float(correct_att) /
                                                                   num_samples), (100.0 * float(adv_correct_att) / num_samples)))

        del data, target, output_att, output_adv_att

    print("Final\t Att_branch Normal: {:.2f}%  \t  "
          "Att_branch Adv: {:.2f}%".format((100.0 * float(correct_att) /
                                            num_samples), (100.0 * float(adv_correct_att) / num_samples)))
