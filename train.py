...
def main(args):
    ...
    train_transforms = transforms.Compose([transforms.Resize(size=(DATA_ARGS['img_size'],
                                                                   DATA_ARGS['img_size'])),
    ...
    train_push_dataset = datasets.ImageFolder(DATA_ARGS['train_push_dir'],
    ...
    test_dataset = datasets.ImageFolder(DATA_ARGS['test_dir'],
                                        transforms.Compose([transforms.Resize(
                                            size=(DATA_ARGS['img_size'], DATA_ARGS['img_size'])),
                                            transforms.ToTensor(), normalize, ]))
    ...
    warm_optimizer_specs = [{'params': ppnet.add_on_layers.parameters(),
                            ...
                            {'params': ppnet.att_layer.parameters(),
                             'lr': warm_optimizer_lrs['att_layer'], 'weight_decay': 1e-3}, ]
    ...
    joint_optimizer_specs = [{'params': ppnet.features.parameters(),
                             ...
                             {'params': ppnet.att_layer.parameters(),
                              'lr': joint_optimizer_lrs['att_layer'], 'weight_decay': 1e-3}]
    ...
    for epoch in range(EXP_ARGS['start_epoch'], EXP_ARGS['num_train_epochs']):
        ...
        if epoch < EXP_ARGS['num_warm_epochs']:
            ...
            _ = tnt.train(model=ppnet_multi, dataloader=train_loader,
            ...
            _ = tnt.train(model=ppnet_multi, dataloader=train_loader,
                          optimizer=joint_optimizer, class_specific=class_specific,
                          coefs=EXP_ARGS['LOSS']['loss_coefs_joint'], log=log, EXP_ARGS=EXP_ARGS)
        ...
        if epoch >= EXP_ARGS['push_start'] and epoch in push_epochs:
            ...
            push.push_prototypes(train_push_loader, prototype_network_parallel=ppnet_multi,
                                 class_specific=class_specific,
            ...
            save.save_model_w_condition(model=ppnet, model_dir=model_dir,
                                        ...
                                        best=True, stage='push_{}'.format(epoch))

            last_layer_optimizer_specs = [{'params': ppnet.last_layer.parameters(),
                                           'lr': EXP_ARGS['OPTIMIZER']['last_layer_optimizer_lrs']
                                           ['last_layer_optimizer_lr']}]

            last_layer_optimizer = torch.optim.Adam(last_layer_optimizer_specs)
            last_lr_lr_scheduler = torch.optim.lr_scheduler.StepLR(last_layer_optimizer,
                                                                   step_size=EXP_ARGS['OPTIMIZER']
                                                                   ['last_layer_optimizer_lrs']
                                                                   ['last_lr_step_size'],
                                                                   gamma=0.1)
            ...
            if NET_ARGS['prototype_activation_function'] != 'linear':
                ...
                for i in range(EXP_ARGS['OPTIMIZER']['last_layer_optimizer_lrs']
                    ...
                    log("Best Accuracy - PostPush {:.2f} at epoch {} and iter {}".format(
                        max_acc_post, max_acc_post_epoch, max_acc_post_iter))

                save.save_model_w_condition(model=ppnet, model_dir=model_dir,
                                            model_name='',
                                            accu=accu, target_accu=0.70, log=log,
                                            best=True, stage='postpushfinal_{}'.format(epoch))
