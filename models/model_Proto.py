...
from .vgg_features import vgg11_features, vgg11_bn_features, vgg13_features,\
...
class PPNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return logits, min_distances, ep
    ...
    def feature_list(self, x):
        ...
        for j in range(len(self.add_on_layers)):
            x = self.add_on_layers[j](x)
            out_list.append(x)
        ...
        prototype_activations = self.distance_2_similarity(min_distances)
        logits = self.last_layer(prototype_activations)
        out_list.append(prototype_activations)
        logits = self.last_layer(prototype_activations)
    ...
    def intermediate_forward(self, x, layer_index):
        ...
        for j in range(len(self.add_on_layers)):
            x = self.add_on_layers[j](x)
            if layer_index == j + len(self.features.features):
                return x
        ...
        if layer_index == len(self.add_on_layers) + len(self.features.features):
